...
В случае если мы хотим поработать чьём то преекте с открытым кодом, мы можем скопировать выбранный репозиторий на наш аккаунт с помощью кнопки FORK в правой верхней части экрана. В своём аккаунте в "форкнутом" репозитории под кнопкой **Code** находим ссылку этого репозитория, переходим в свой локальный git и используем команду git clone <ссылка удалённого репозитория>.
После этого необходимо создать новую ветку, в которой и будут проходить ваши изменения.

По окончании работы нужно будет запушить все изменения обратно к себе на GitHub аккаунт, а уже после этого сделать запрос автору форкнутого репозитория на добавление изменений с помощью кнопки PULL REQUEST
